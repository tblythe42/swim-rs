#!/usr/bin/env python
"""
PEST++ Calibration Script for Crane
====================================
...
def main():
    parser = argparse.ArgumentParser(
        description="Run PEST++ IES calibration for Crane",
    ...
    parser.add_argument(
        "--noptmax", type=int, default=3, help="Number of optimization iterations (default: 3)"
    )
    parser.add_argument(
        "--reals",
        type=int,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Build files only, do not run PEST++"
    )
    parser.add_argument(
        "--skip-build", action="store_true", help="Skip build step, run existing PEST++ setup"
    )
    parser.add_argument(
        "--no-cleanup",
        action="store_true",
    ...
    parser.add_argument("--no-archive", action="store_true", help="Skip the cleanup/archiving step")
    ...
    try:
        ...
        if args.dry_run:
            ...
            # Run calibration
            run_calibration(
                builder, cfg, workers=args.workers, cleanup=not args.no_cleanup, verbose=True
            )

            # Process results: summarize, check success, cleanup/archive
            success = process_results(
                builder, cfg, archive_dir=args.archive, skip_archive=args.no_archive
            )

            if success:
                print("\nCalibration complete!")
                print("Use calibrated parameters in 03_calibrated_model.ipynb")
            else:

"""Evaluate full merged LS/ECO calibration against flux tower ET.

Reads calibrated parameters from the container, runs a single forward
model pass for all 234 calibrated sites, and compares daily ET against
flux tower ET_corr.  Reports overall and LULC-stratified metrics plus
a head-to-head SWIM vs Landsat PT-JPL comparison.

Usage:
    uv run python examples/6_Flux_International/evaluate_merged_full.py
"""
...
TOML = Path(__file__).resolve().parent / "full_run.toml"
...
def _extract_params_from_container(container: SwimContainer) -> dict[str, dict[str, float]]:
    """Read calibrated params from container zarr arrays."""
...
def main():
    ...
    print(f"  {len(sites)} calibrated sites")
    ...
    try:
        ...
        print(f"Running forward model ({swim_input.n_fields} sites × {swim_input.n_days} days)...")
    ...
    # Build site->column index for forward-run output
    site_idx = {sid: j for j, sid in enumerate(sites)}

    # Landsat PT-JPL ETf and ETo from container for head-to-head
    root = container._root
    ls_etf_all = root["remote_sensing/etf/landsat/ptjpl/no_mask"][:]
    eto_all = root["meteorology/era5/eto"][:]
    container.close()

    print(f"\nComputing metrics vs flux tower ET_corr ({len(sites)} sites)...")
    print(f"{'SID':<10}  {'--- SWIM ---':^35}  {'--- Landsat PT-JPL (obs dates) ---':^35}")
    print(
        f"{'':10}  {'n':>5} {'bias':>7} {'rmse':>7} {'r2':>7}  "
        f"{'n':>5} {'bias':>7} {'rmse':>7} {'r2':>7}"
    )
    print("-" * 88)
    ...
    for sid in sites:
        ...
        # On Landsat obs dates: SWIM, Landsat, flux all matched
    ...
    out_csv = OUT_DIR / "merged_full_evaluation.csv"
    ...
    hdr = (
        f"{'LULC':<12} {'n':>3}  "
        f"{'SWIM bias':>9} {'SWIM rmse':>9} {'SWIM r2':>7} {'SWIM KGE':>8}  "
        f"{'LS bias':>9} {'LS rmse':>9} {'LS r2':>7} {'LS KGE':>8}  {'winner':>6}"
    )
    print(hdr)
    print("-" * len(hdr))
    for lc_name, grp in sorted(valid.groupby("lulc_name"), key=lambda x: -len(x[1])):
        sw_b = grp["swim_ls_bias"].median()
        sw_rm = grp["swim_ls_rmse"].median()
        sw_r = grp["swim_ls_r2"].median()
        sw_k = grp["swim_ls_kge"].median()
        ls_b = grp["ls_bias"].median()
        ls_rm = grp["ls_rmse"].median()
        ls_r = grp["ls_r2"].median()
        ls_k = grp["ls_kge"].median()
        winner = "SWIM" if sw_k >= ls_k else "LS"
        print(
            f"{lc_name:<12} {len(grp):3d}  "
            f"{sw_b:+9.3f} {sw_rm:9.3f} {sw_r:7.3f} {sw_k:8.3f}  "
            f"{ls_b:+9.3f} {ls_rm:9.3f} {ls_r:7.3f} {ls_k:8.3f}  {winner:>6}"
        )

    # Overall KGE summary
    print(
        f"\nOverall median KGE:  SWIM={valid['swim_ls_kge'].median():.3f}  "
        f"Landsat={valid['ls_kge'].median():.3f}"
    )
    print(f"Saved: {out_csv}")

    # Scatter plot: SWIM vs Landsat per-site metrics, colored by LULC
    lulc_colors = {
        "Cropland": "#e6a817",
        "CropNatMos": "#daa520",
        "DBF": "#2ca02c",
    ...
    fig.suptitle(
        f"Merged LS/ECO full calibration — SWIM vs Landsat PT-JPL ({len(valid)} sites)", fontsize=11
    )
    ...
    for i, (swim_col, ls_col, xlabel, title) in enumerate(
        [
            ...
            ("swim_ls_kge", "ls_kge", "KGE", "KGE vs flux"),
        ]
    ):
    ...
    plt.tight_layout()
    out_png = OUT_DIR / "merged_full_evaluation.png"

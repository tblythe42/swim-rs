...
import numpy as np
from numpy import abs, argmin, array, datetime64, empty, float32, timedelta64
...
class TopoWX(Thredds):
    ...
    def get_data_subset(
        ...
        self.src_bounds_wsen = west_val, south_val, east_val, north_val
    ...
    def _build_url(self, var: str) -> str:
        ...
        url = urlunparse(
            [
                self.scheme,
                self.service,
                f"/thredds/dodsC/topowx?crs,lat[0:1:3249],lon[0:1:6999],{var},time",
                "",
                "",
                "",
            ]
...
class GridMet(Thredds):
    ...
    def __init__(
        ...
        if self.variable not in self.available:
            Warning(f"Variable {self.variable} is not available")
    ...
    def subset_daily_tif(self, out_filename: str | None = None) -> np.ndarray:
        ...
        if self.variable == "elev":
            ...
            self.width = subset.dims["lon"]
            self.height = subset.dims["lat"]

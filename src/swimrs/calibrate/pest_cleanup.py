...
class PestResults:
    ...
    def is_successful(self) -> tuple[bool, list[str]]:
        ...
        has_completion = any(p.lower() in rec_content.lower() for p in completion_patterns)
    ...
    def _get_recommendations(self, issues: list[str]) -> list[str]:
        ...
        for issue in issues:
            if "record file not found" in issue.lower():
                ...
                recommendations.append("Try increasing realizations or adjusting parameter bounds.")
        ...
        if not recommendations:
            recommendations.append("Check worker_0/panther_worker.rec for detailed error messages.")
            recommendations.append("Verify observation files exist in obs/ directory.")
        ...
        if "parameters" in summary:
            ...
            for base, items in sorted(param_groups.items()):
                for name, vals in items:
                    print(
                        f"  {name}: {vals['initial']:.3f} -> {vals['final']:.3f} "
...
def main() -> int:
    ...
    parser.add_argument(
        "--archive", "-a", default=None, help="Archive directory (default: pest_dir/archive)"
    )
    parser.add_argument(
        "--check-only", action="store_true", help="Check status and print summary without cleanup"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be cleaned without doing it"
    )
    parser.add_argument(
        "--keep-debug", action="store_true", help="Keep debug files even on success"
    )

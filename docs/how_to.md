...
The full annotated template is shown below. You can also [download template.toml](template.toml) directly.

??? note "template.toml (click to expand)"
...
See [Config Schema Reference](#config-schema-reference) below for detailed documentation of all options.
...
- See the [template.toml](#step-3-create-the-toml-config) embedded above for a complete working example
